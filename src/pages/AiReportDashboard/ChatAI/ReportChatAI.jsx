...
function ReportChatAI() {
  ...
  const testId = decodeURIComponent(searchParamsIDS.get('id'))?.split('&')[0]?.split('#')?.filter(Boolean)
console.log("testIdtestIdtestIdtestIdtestIdtestIdtestIdtestIdtestIdtestIdtestId", testId)
  ...
  const getSuggation = async (clientCode) => {
    ...
    if (apiResp?.success) {
      setSuggestions([{ ID: 0, Question: 'Download Report !' }, ...apiResp?.data])
    } else {
  ...
  const reCheckMessages = async () => {
    ...
    const apiResp = await axios.post(`${apiURL}LabReport/AIGetSavedResponse`, payload)
    if (apiResp?.data?.success) {
      setApiDataDetail((val) => ({
        ...val, reCheckMessage: apiResp?.data?.data,
        isDisabled: apiResp?.data?.data?.length >= reportDetails?.MaximumQuestionAllow ? true : false
      }))
    } else {
      setApiDataDetail((val) => ({
        ...val, reCheckMessage: [],
  ...
  const saveAiMessage = async (requestType = "Request", query = "", response = "") => {
    ...
    const apiResp = await axios.post(`${apiURL}LabReport/AIRequestDetailsInsert`, payload)
    reCheckMessages()
    return apiResp?.data
  ...
  const getResportData = async () => {
    try {
      ...
      const respData = await axios.post(`${apiURL}LabReport/LabGetReportValue`, payload)
      ...
      if (respData?.data?.success) {
        ...
        input.forEach(item => {
          ...
          groupedTests[TestName][LabObservationName] = isNaN(VALUE) ? VALUE : parseFloat(VALUE);
        });
  ...
  const handleSubmit = async (data, ID) => {
    ...
    if (!data.trim()) return;
    setInput("");
    const saveResp = apiDataDetail?.reCheckMessage?.find((val) => val?.Query === data.trim())
  ...
  const checkRemQuery = () => {
    // debugger
    if (reportDetails?.MaximumQuestionAllow - apiDataDetail?.reCheckMessage?.length < 0) {
      return 0
    } else {
      return reportDetails?.MaximumQuestionAllow - apiDataDetail?.reCheckMessage?.length
    }
  }
  ...
  const handleSeeHistory = () => {
    ...
    apiDataDetail?.reCheckMessage?.map((val) => {
      newData.push({ role: 'ai-chat-user', content: val?.Query },
        ...
        <div className="chat-header-left">
          ...
          <div className="chat-header-text">
            ...
            <div className="chat-header-subtitle" style={{ marginTop: "1px" }}>
              ...
              {!isNaN(checkRemQuery()) &&
                <span className="chat-header-subtitle" style={{ padding: "2px 5px", borderRadius: "3px", marginTop: "1px", marginLeft: "6px", opacity: 1, background: checkRemQuery() === 0 ? "green" : checkRemQuery() === 1 ? "#b93b3b" : checkRemQuery() ? "#acac2d" : "" }}>
                  {checkRemQuery() === 0 ? "Query Completed" : `Query Remaining ${checkRemQuery()}`}
                </span>
              }
            </div>
          </div>
        ...
        <div className="chat-suggestions">
          {suggestions.map((sug, i) => (
            <button
              ...
              onClick={(e) => { handleSubmit(sug?.Question, sug?.ID) }}
      ...
      {!apiDataDetail?.isDisabled &&
        <div className="chat-input-wrapper">
          ...
          <input
            ...
            disabled={isTyping || apiDataDetail?.isDisabled || !reportDetails?.IsOpenQuestionAllow}
          />
